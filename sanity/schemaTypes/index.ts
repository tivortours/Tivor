...
const destination = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description: "Journeys shown in the 'Crafted Journeys' carousel on this destination's page, in this order — drag to reorder. Also controls the 'Other Journeys' order on those journeys' own detail pages. Leave empty to automatically show every journey tagged with this destination, sorted by that journey's Sort Order field.",
    }),
...
const journey = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "boolean",
      initialValue: false,
      description: "Fallback only — used when the Home Page's Featured Journeys list is empty. To control which journeys show and in what order, add them to that list instead.",
    }),
...
const inspirationArticle = defineType({
  ...
  orderings: [orderRankOrdering],
  fields: [
    orderRankField({ type: "inspirationArticle" }),
    defineField({
      name: "active",
      title: "Active",
      type: "boolean",
      initialValue: true,
      description: "Uncheck to hide this article from the Inspiration page without deleting it.",
    }),
    defineField({ name: "title", title: "Title", type: "string", validation: (Rule) => Rule.required() }),
    defineField({ name: "slug", title: "Slug", type: "slug", options: { source: "title" }, validation: (Rule) => Rule.required() }),
...
const homePage = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "array",
      of: [defineArrayMember({ type: "reference", to: [{ type: "journey" }] })],
      description: "Journeys shown here, in this order — drag to reorder. If left empty, falls back to journeys with 'Feature on Home Page' checked.",
    }),
